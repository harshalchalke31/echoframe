...
class MobileNetV3Encoder(nn.Module):
    def __init__(self, config_name="large", inchannels=3, backbone=False):
        ...
        if self.backbone:
            ...
            # If we want 4 input channels, adapt the first conv layer
            if inchannels != 3:
                self._adapt_input_channels(inchannels)
    ...
    def _adapt_input_channels(self, inchannels):
        # The first conv layer is typically self.backbone_model.features[0][0]
        first_conv = self.backbone_model.features[0][0]
        old_weights = first_conv.weight.data  # shape (out_channels, 3, kh, kw)
        out_channels, old_in_channels, kh, kw = old_weights.shape

        # Create new weight with 'inchannels' input channels
        new_weights = torch.zeros((out_channels, inchannels, kh, kw))

        # Copy the original 3 channels
        new_weights[:, :3, :, :] = old_weights

        # For extra channels, replicate channel 0
        for extra_ch in range(3, inchannels):
            new_weights[:, extra_ch, :, :] = old_weights[:, 0, :, :]

        first_conv.weight = nn.Parameter(new_weights)
